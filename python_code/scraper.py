...
# attributes = ['Course id', 'English title', 'Danish title', 'Point( ECTS )', 'Course type',
#               'Location', 'Recommended prerequisites', 'Responsible', 'Department', 'Link']
attributes = ['Kursus id', 'Engelsk titel', 'Dansk titel', 'Point( ECTS )', 'Kursustype',
              'Undervisningens placering', 'Anbefalede forudsætninger', 'Kursusansvarlig', 'Institut', 'Link']
df = pd.DataFrame(None, columns=attributes)
...
for course_link in soup.find('table', {'class': 'table'}).find_all('a'):
    ...
    course_data = {k: None for k in attributes}
    ...
    # course_data['Course id'] = raw_eng_title[2:7]
    # course_data['English title'] = raw_eng_title[8:]
